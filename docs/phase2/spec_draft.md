...
for any directory except MPW (Task D). Task E sampling will resolve this --
if some topology sub-types share template structure, the count drops toward
35. If they genuinely differ, it stays near 55.
...
class InitStrategy(Enum):
    NODESET_ONLY = "nodeset"       # .nodeset for Q/QN (standard FF)
    NODESET_LATCH = "nodeset_latch" # .nodeset + internal latch state
    IC_RETENTION = "ic_retention"  # .ic for retention mode entry/exit
...
| # | Cell family | Topology class | Init strategy | Measurement | Why |
|---|-------------|---------------|---------------|-------------|-----|
| 1 | Standard FF (DFFQ1, SDFQ1) | COMMON | nodeset (Q/QN) | pushout | Baseline: simplest hold arc |
| 2 | Latch (LHQD1) | LATCH | nodeset_latch | glitch (maxq/minq) | Tests latch init + glitch polarity dispatch |
| 3 | Multi-bank (MB*SRLSDF*) | MB | nodeset_latch (extended) | glitch | Tests multi-bank init + multi-input expansion (AO22) |
| 4 | Synchronizer (SYNC2-6) | SYNC | nodeset | pushout | Tests depth parameterization + waveform scaling |
| 5 | Clock gater (CKGAN2*, CKGND2*) | CKG | nodeset | pushout + nochange | Tests gate-type sub-classification (AND vs NAND). Note: nochange arcs are out of MVP scope but the CKG classifier must still work for hold arcs on these cells |

These 5 families exercise:
- All 3 init strategies
...
### Phase 2C: Setup + MPW + Remaining Arc Types [M]

**Delivers**:
- Setup arc support (trivial: reuse hold families with different timing)
- MPW arc support (63 shipped templates already validated)
- removal, non_seq_hold, non_seq_setup, nochange support
- Retention cell support (RETN*, ic_retention init strategy)
...
**Complexity**: M (medium). Most of the architecture is in place from 2B.
This is extending families + testing. Nochange templates have a different
waveform model which adds one new axis.

**Sequencing**: After 2B. Can potentially parallelize with 2D.
...
|-----------|----------|------|------------|
| 2A | Classifier + selector + regression harness | M | Task E results |
| 2B | Param binder + hold deck generation | L | 2A |
| 2C | All arc types + retention + nochange | M | 2B |
| 2D | GUI integration + documentation | S | 2C (partial parallel) |
